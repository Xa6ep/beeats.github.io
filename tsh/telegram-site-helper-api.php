<?php
	...
	if(!array_key_exists("act", $_GET)){
		echo json_encode(array("status"=>"error", "error"=>"NO_ACT"));
		exit();
	}
	...
	switch ($act) {
		...
		case 'newChat':
			...
			$sth->execute(array(":managerStatus"=>1));
			$managers=array();
			...
			if(count($managers)==0){
				echo json_encode(array("status"=>"error","error"=>"NO_MANAGERS_AVALIABLE"));
				exit();
			}
			$randomManagerId=$managers[rand(0,(count($managers)-1))];
			...
			$sth->execute(array(":chatId"=>$chatId, ":chatManager"=>$randomManagerId, ":chatCustomerName"=>$chatCustomerName, ":chatCustomerPhone"=>$chatCustomerPhone));
			$sth=$db->prepare("SELECT * FROM telegramSiteHelperManagers WHERE managerId=:managerId");
			$sth->execute(array(":managerId"=>$randomManagerId));
			$answer=$sth->fetch();
			sendMessage(BOTTOKEN,$answer["managerTelegramId"],"Новый клиент".$client." начал чат. Для перехода к чату нажмите /chat_".$chatId." или дождитесь сообщения от клиента.");
			echo json_encode(array("status"=>"ok","chatId"=>$chatId,"manager"=>$answer["managerName"]));
			exit();
		break;

		case 'editChat':
			if(!array_key_exists("chatId", $_POST)){
				echo json_encode(array("status"=>"error","error"=>"NO_CHAT_ID"));
				exit();	
			}	
			$chatId=$_POST["chatId"];
			if(array_key_exists("chatCustomerName", $_POST)){
			...
			$sth->execute(array(":chatId"=>$chatId, ":chatCustomerName"=>$chatCustomerName, ":chatCustomerPhone"=>$chatCustomerPhone));
			...
			$sth->execute(array(":managerNowChat"=>$chatId));
			while($answer=$sth->fetch()){
				$managerToSend[]=$answer["managerTelegramId"];
			}
			$managerToSend=array_unique($managerToSend);
			foreach ($managerToSend as $i => $item) {
				sendMessage(BOTTOKEN,$item,"Клиент из чата /chat_".$chatId." оставил данные: ".$chatCustomerName." ".$chatCustomerPhone);
			}
		...
		case 'sendMessage':
			...
			if(!array_key_exists("message", $_POST) AND !array_key_exists("file", $_POST)){
				echo json_encode(array("status"=>"error","error"=>"NO_MESSAGE"));
				exit();	
			}
			$message=null;
			...
			if($message!=null){
				...
				$managerToSend=array();
				if($answer["managerTelegramId"]!=null){
					$managerToSend[]=$answer["managerTelegramId"];
				}
				$sth=$db->prepare("SELECT managerTelegramId FROM telegramSiteHelperManagers WHERE managerNowChat=:managerNowChat");
				$sth->execute(array(":managerNowChat"=>$chatId));
				...
				if(count($managerToSend)==0){
					echo json_encode(array("status"=>"error","error"=>"NO_MANAGER"));
					exit();						
				}
				foreach ($managerToSend as $i => $item) {
				...
				$msgText=json_encode(array("text"=>$message));
				$sth->execute(array(":msgChatId"=>$chatId, ":msgFrom"=>"client", ":msgTime"=>time(), ":msgText"=>$msgText));
			...
			if($file!=null){
				...
				if(mb_strlen($file)>10240000){
					echo json_encode(array("status"=>"error", "error"=>"SO_BIG_FILE"));
					exit();
				}
				...
				unlink($fileURL);
			 	...
			 	if($ft=="photo"){
					$msgText=json_encode(array("photo"=>$messageFileId, "filename"=>""));
				}else{
					$msgText=json_encode(array("file"=>$messageFileId, "filename"=>$messageFileName));	
				}
				$sth=$db->prepare("INSERT INTO telegramSiteHelperMessages (msgChatId, msgFrom, msgTime, msgText) VALUES (:msgChatId, :msgFrom, :msgTime, :msgText)");
				$sth->execute(array(":msgChatId"=>$chatId, ":msgFrom"=>"client", ":msgTime"=>time(), ":msgText"=>$msgText));
			...
			echo json_encode(array("status"=>"ok", "time"=>date("j.m (H:i:s)",time())));
			exit();	
		break;
 		...
 		case 'pollMessages':
			...
			if(!array_key_exists("chatId", $_GET)){
				...
				echo json_encode(array("command"=>"error", "error"=>"NO_CHAT_ID"));
			...
			if($lastMessageId==0 OR $type=="sse"){
				...
				if($answer["count"]==0){
					...
					echo json_encode(array("command"=>"error", "error"=>"BAD_CHAT_ID"));
					if($type=="sse"){
						echo "\n\n";
					};					
					exit();					
				...
				echo json_encode(array("command"=>"allMessages", "messages"=>array_reverse($messages), "lastMessageId"=>$lastMessageId));
				...
				if($type=="sse"){
					echo "data: ";
					echo json_encode(array("command"=>"loadComplete"))."\n\n";
					@ob_flush();
					flush();
				}
			}
			...
			while(true){
				...
				if($lastTimeUpdate!=false){
					...
					if($lastTimeUpdate>$lastTimeQuery){
						...
						$sth->execute(array(":msgChatId"=>$chatId,":msgId"=>$lastMessageId));
						$messages=array();
						...
						if(count($messages)>0){
							...
							echo json_encode(array("command"=>"newMessages", "messages"=>$messages,"lastMessageId"=>$lastMessageId));
							if($type=="sse"){
								echo "\n\n";
								@ob_flush();
								flush();
				...
				if($type=="lp"){
					if((time()-$nowTime)>60){
						echo json_encode(array("command"=>"timeout"));
						exit();
					}
				}
			}
		...
		default:
			echo json_encode(array("status"=>"error","error"=>"BAD_ACT"));
			exit();	
